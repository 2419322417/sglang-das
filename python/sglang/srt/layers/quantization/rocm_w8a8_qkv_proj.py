...
"""Exact-shape gfx928 W8A8 self_attn.qkv_proj M=1 integration."""
...
_PREFIX_SUFFIX = "self_attn.qkv_proj"
_INPUT_SHAPE = (1, 6144)
_WEIGHT_SHAPE = (1280, 6144)
_PACKED_SHAPE = (1536, 1280)
_OUTPUT_SHAPE = (1, 1280)
_PACKED_BUFFER = "w8a8_gfx928_qkv_proj_m1_k4n"
_SCALE_BUFFER = "w8a8_gfx928_qkv_proj_m1_weight_scale"
_ACC_BUFFER = "w8a8_gfx928_qkv_proj_m1_acc"
_COUNTER_BUFFER = "w8a8_gfx928_qkv_proj_m1_counter"
_OP_NAME = "w8a8_gfx928_qkv_proj_m1"

if torch.version.hip is not None:
    import sgl_kernel  # noqa: F401 -- registers the custom op

    @register_fake_if_exists(f"sgl_kernel::{_OP_NAME}")
    def _w8a8_gfx928_qkv_proj_m1_fake(
        x,
        weight_k4n,
        weight_scale,
        bias,
...
def pack_signed_int8_k4n(weight_nk: torch.Tensor) -> torch.Tensor:
    ...
    if weight_nk.dtype != torch.int8 or tuple(weight_nk.shape) != _WEIGHT_SHAPE:
        raise ValueError("expected signed int8 checkpoint weight [1280, 6144]")
...
def initialize_qkv_proj_m1(layer: torch.nn.Module) -> None:
...
def install_qkv_proj_m1(
    ...
    *,
    ...
    if (
        not envs.SGLANG_ROCM_GFX928_W8A8_QKV_PROJ_M1.get()
    ...
    setattr(
        layer,
        _ACC_BUFFER,
        torch.zeros(1280, dtype=torch.int32, device=weight.device),
    )
    setattr(
        layer,
        _COUNTER_BUFFER,
...
def try_qkv_proj_m1(
    ...
    """
    packed = layer.w8a8_gfx928_qkv_proj_m1_k4n
    weight_scale = layer.w8a8_gfx928_qkv_proj_m1_weight_scale
    acc = layer.w8a8_gfx928_qkv_proj_m1_acc
    counter = layer.w8a8_gfx928_qkv_proj_m1_counter
    if (
        not envs.SGLANG_ROCM_GFX928_W8A8_QKV_PROJ_M1.get()
        or not prefix.endswith(_PREFIX_SUFFIX)
    ...
    return torch.ops.sgl_kernel.w8a8_gfx928_qkv_proj_m1(
        x,
        packed,
        weight_scale,
        bias,
