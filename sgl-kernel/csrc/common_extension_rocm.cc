...
TORCH_LIBRARY_EXPAND(sgl_kernel, m) {
  ...
  m.def(
      "transfer_kv_all_kernel_lf_pf_D2H_hcu(Tensor src_k, Tensor dst_k,Tensor src_v, Tensor dst_v,"
      "Tensor src_indices,Tensor dst_indices, int item_size, int srt_layout_dim,int dst_layout_dim, int page_size,int "
      "layer_num, int num_warps_per_block) ->() ");
  m.impl("transfer_kv_all_kernel_lf_pf_D2H_hcu", torch::kCUDA, &transfer_kv_all_kernel_lf_pf_D2H_hcu);
  m.def(
      "transfer_kv_per_layer_kernel_pf_lf_H2D_hcu(Tensor src_k, Tensor dst_k,Tensor src_v, Tensor dst_v,"
      "Tensor src_indices,Tensor dst_indices, int item_size, int src_layout_dim,int page_size,int layer_id, int "
      "num_warps_per_block) ->() ");
  m.impl("transfer_kv_per_layer_kernel_pf_lf_H2D_hcu", torch::kCUDA, &transfer_kv_per_layer_kernel_pf_lf_H2D_hcu);
  m.def(
      "transfer_kv_all_direct_pf_lf_H2D_hcu(Tensor src_ptrs_k, Tensor src_ptrs_v,Tensor[] dst_ptrs_k, Tensor[] "
      "dst_ptrs_v,"
      "Tensor src_indices,Tensor dst_indices, int start_layer_id, int page_size) ->() ");
  m.impl("transfer_kv_all_direct_pf_lf_H2D_hcu", torch::kCUDA, &transfer_kv_all_direct_pf_lf_H2D_hcu);
  m.def(
      "transfer_kv_all_direct_lf_pf_D2H_hcu(Tensor[] src_ptrs_k, Tensor[] src_ptrs_v,Tensor dst_ptrs_k, Tensor "
      "dst_ptrs_v,"
      "Tensor src_indices,Tensor dst_indices, int start_layer_id, int page_size) ->() ");
